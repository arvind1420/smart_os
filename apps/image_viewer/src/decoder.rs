//! PNG + JPEG + BMP image decoder for user-space.
//! Ported from kernel/src/drivers/image.rs
...
pub fn decode_png(data: &[u8]) -> Result<Image, ImageError> {
    ...
    let stride = ((width as usize * channels * bit_depth as usize + 7) / 8) + 1;
...
pub fn decode_jpeg(data: &[u8]) -> Result<Image, ImageError> {
    ...
    while si < sos_data.len() {
        ...
        if b == 0xFF {
            ...
            if next == 0x00 { si += 1; }
            else if next == 0xD9 { break; }
        }
    }

    let mut br = BitReader::new(&ecd);
    let mut img = Image::new(width, height);
    let ncomp = comp_info.len().min(3);
...
pub fn decode_bmp(data: &[u8]) -> Result<Image, ImageError> {
    ...
    if comp != 0 && comp != 3 { return Err(ImageError::UnsupportedFormat); }
    if bpp != 24 && bpp != 32 { return Err(ImageError::UnsupportedFormat); }

    let bytes_pp = bpp as usize / 8;
    let row_stride = (width as usize * bytes_pp + 3) & !3;
...
pub fn init() {
    smartsdk::io::print("[image] PNG/JPEG/BMP image decoder ready.\n");
}
